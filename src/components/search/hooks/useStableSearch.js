...
export const useStableSearchWithDistance = (options = {}) => {
    ...
    const addDistanceToStable = useCallback(async (stable, userLocation) => {
        ...
        if (stable.postCode) {
            try {
                const locationResponse = await axiosInstance.get(`/api/stable-location/${stable.postCode}`);
        ...
        // If stable doesn't have postcode, get the full stable details
            ...
            try {
                ...
                if (response.isSuccess && response.value?.postCode) {
                    ...
                    console.log("Got stable details with postcode:", fullStable.postCode);

                    // Now get the location data
                    const locationResponse = await axiosInstance.get(`/api/stable-location/${fullStable.postCode}`);

                    if (locationResponse.isSuccess && locationResponse.value) {
                        const locationData = locationResponse.value;

                        if (locationData.latitude && locationData.longitude) {
                            console.log("Got location data with coordinates:", locationData);
                    ...
                    console.log("Stable details missing postcode or request failed:", response);
                }
            } catch (error) {
            ...
            searchFn: async (query) => {
                ...
                if (results.success && userLocation) {
                    ...
                    const enhancedStables = await Promise.all(
                        results.data.map(stable => addDistanceToStable(stable, userLocation))
                    );

...
const MemoizedListItem = React.memo(ListItemRenderer, (prev, next) => {
  return (
    ...
    prev.item[prev.config?.idField || "id"] ===
      next.item[next.config?.idField || "id"]
  );
});
...
const SearchResults = ({
  ...
  useEffect(() => {
    if (loading) {
      // Show results immediately when loading starts
  ...
  useEffect(() => {
    ...
    if (
      resultsRef.current &&
      results.length > 0 &&
      results.length !== prevQueryRef.current
    ) {
  ...
  return (
        ...
        <FormMessage
          message={
            typeof error === "string"
              ...
              : error?.text
              ? error
              : { type: "error", text: "An error occurred during search" }
          }
        />
      </div>
    ...
    transition-opacity duration-300 ${
      !loading && message ? "opacity-100" : "opacity-0"
    }`}
        role="alert"
        style={{
      ...
      >
        <div>
          ...
          {message &&
            message.type === "error" &&
            message.text &&
            message.text.includes("Can't find") && (

...
export const useHorseManagement = (stableId) => {
    ...
    const fetchHorses = useCallback(async () => {
        ...
        if(emptyStablesCache.has(stableId)) {
            console.log(`Stable ${stableId} is known to be empty - skipping API call`);
            setHorses([]);
            setError(null);
            setLoading(false);
            return;
        ...
        try {
            ...
            const horsesData = await horseService.getHorsesWithOwners(stableId);

            // Map to the expected format in the UI
            const processedHorses = Array.isArray(horsesData) ? horsesData.map(horse => ({
                id: horse.horseId,
                stableHorseId: horse.id,
                name: horse.horseName,
                color: horse.horseColor,
                owner: horse.horseOwners && horse.horseOwners.length > 0 ? {
            ...
            if(errorMessage.includes("No horses")) {
                emptyStablesCache.add(stableId);
                console.log("Caught 'No horses' error in hook - setting horses to empty array");
                setHorses([]);
                setError(null);
                setLoading(false);
                return;
    ...
    const addHorse = async (horseData, userId) => {
        ...
        try {
            console.log(`Adding horse to stable ID: ${stableId} for user ID: ${userId}`);
